	...
	public function handle(string $uri)
	{
		try {
			...
			if (
				...
				$exception = new \CartApp\Core\Exception\BaseException('Handler not found', 404, $exception);
			} elseif (
				$exception instanceof \Phalcon\Mvc\Dispatcher\Exception
				&& $exception->getCode() === \Phalcon\Dispatcher\Exception::EXCEPTION_ACTION_NOT_FOUND
			) {
				$exception = new \CartApp\Core\Exception\BaseException('Action not found', 404, $exception);
			} elseif (!($exception instanceof \CartApp\Core\Exception\BaseException)) {
				$exception = new \CartApp\Core\Exception\BaseException($exception->getMessage(), 500, $exception);
			}
